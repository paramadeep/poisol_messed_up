module ClassTemplate
  ...
  def ClassTemplate.included (stub_class)
    binding.pry
    @config = ConfigMap.file stub_class.class_name
    prepare_request
    prepare_response
  end

  def ClassTemplate.prepare_request
    prepare_request_url
    prepare_request_query
    prepare_request_body
  end

  def ClassTemplate.prepare_request_url
    @url = @config.request.url.clone
    @type = @config.request.type.clone
  end

  def ClassTemplate.prepare_request_query
    @query = (@config.request.query.nil?) ? "" : generate_query_methods
  end

  def ClassTemplate.prepare_request_body
